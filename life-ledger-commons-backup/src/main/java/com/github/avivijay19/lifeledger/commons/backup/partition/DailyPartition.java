...
public final class DailyPartition implements PartitionStrategy {
    ...
    public Path resolve(String domain, String tableName, Instant backupTime) {
        ...
        var month = String.format("%02d", zdt.getMonthValue());
        var day = String.format("%02d", zdt.getDayOfMonth());
        var fileName = tableName.toLowerCase().replace("_", "-")
            + "-" + zdt.format(FILE_TS) + ".parquet";
        return Path.of(domain, tableName.toLowerCase().replace("_", "-"),
